...
class OmniglotTask(object):
    ...
    def __init__(self, train_val_permutation, root=DATA_DIR, num_cls=5, num_inst=1, train=True):
        ...
        root_zip1  = zipfile.Zipfile(os.path.join(root, 'images_background') + '.zip')
        print(root_zip1)
        root_zip2 = zipfile.Zipfile(os.path.join(root, 'images_evaluation') + '.zip')
        ...
        languages1 =  [f for f in root_zip1.namelist() if f.startswith('subdir') and f.count("/")==1]#os.listdir(self.root1)
        languages2 = [f for f in root_zip2.namelist() if f.startswith('subdir') and f.count("/")==1]  #os.listdir(self.root2)
        languages1.sort()
        languages2.sort()
        chars = []
        for l in languages1:
            chars += [os.path.join(self.root1, l, str(x)) for x in os.listdir(os.path.join(self.root1, l))]
        for l in languages2:
            chars += [os.path.join(self.root2, l, str(x)) for x in os.listdir(os.path.join(self.root2, l))]
        ...
        for c in classes:
            ...
            #print('ttrain id', self.train_ids)
            self.val_ids += instances[c][num_inst:num_inst * 2]
        # Keep instances separated by class for class-balanced mini-batches
        self.train_labels = [labels[self.get_class(str(x))] for x in self.train_ids]
        self.val_labels = [labels[self.get_class(str(x))] for x in self.val_ids]
...
class OmniglotFewShotDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for idx, file_name in enumerate(task_def.train_ids):
            ...
            image = Image.open(str(image_path), mode='r').convert('L')

...
class Learner:
    ...
    def learn_task(self, task, num_steps=10, add_regularization=False, w_0=None, lam=0.0, tr_val=True):
        if tr_val:
            xt, yt = task['x_train'], task['y_train']
        else:
            xt = torch.cat( ( task['x_train'], task['x_val'] ), 0)
            yt = torch.cat( ( task['y_train'], task['y_val']), 0)
    ...
    def matrix_evaluator(self, task, lam, regu_coef=1.0, lam_damping=10.0, x=None, y=None, tr_val=True):
        """
        ...
        def evaluator(v):
            hvp = self.hessian_vector_product(task, v, x=x, y=y, tr_val=tr_val)
            Av = (1.0 + regu_coef) * v + hvp / (lam + lam_damping)
            return Av
        return evaluator

    def hessian_vector_product(self, task, vector, params=None, x=None, y=None, tr_val=True):
        """
